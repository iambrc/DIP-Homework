...
import draggan.draggan as draggan
from draggan.draggan import drag_gan
from draggan.draggan import utils
...
def on_click(image, target_point, points, size, preds, evt: gr.SelectData):
    if target_point:
    ...
    for pred in preds:
        for i in range(len(pred)):
            if (pred[i, 0] - evt.index[0]) ** 2 + (pred[i, 1] - evt.index[1]) ** 2 < 25:
                near_pred = [pred[i, 0], pred[i, 1]]
                flag = True

    if flag:
        points['handle'].append([near_pred[1], near_pred[0]])
    else:
...
def on_image_change(model, image_size, image):
    ...
    points = {'target': [], 'handle': []}
    target_point = False
    return image, image, result, points, target_point, preds
...
def main():
    ...
    with gr.Blocks() as demo:
        ...
        image.select(on_click, [image, target_point, points, size, preds], [image, target_point])
        image.upload(on_image_change, [model, size, image], [image, mask, state, points, target_point, preds])
        mask.upload(on_mask_change, [mask], [image])
        btn.click(on_drag, inputs=[model, points, max_iters, state, size, mask, lr_box],
        ...
        model_dropdown.change(on_change_model, inputs=[model_dropdown, model],
                              outputs=[model, state, image, mask, size])
